...
from src.processor import HandResult  # noqa: E402
...
class TestCoordinateMapper:
    ...
    def test_deadzone_prevents_micro_movement(self) -> None:
        mapper = self._make_mapper(deadzone_px=10, smoothing_alpha=1.0)
    ...
    def test_smoothing_reduces_jitter(self) -> None:
        mapper = self._make_mapper(smoothing_alpha=0.3, deadzone_px=0)
        # Initial position
        mapper.map(320, 240)
        # Jump to a far position
        sx, _ = mapper.map(500, 240)
        # With low alpha, the smoothed position should not reach the target
        target_sx, _ = CoordinateMapper(
            camera_width=640, camera_height=480,
            active_region=0.7, smoothing_alpha=1.0,
            deadzone_px=0, mirror_x=False,
        ).map(500, 240)
        # Smoothed should be between initial and target
        assert sx < target_sx
...
class TestHandController:
    ...
    def test_idle_to_cursor_on_hand_detect(self, controller: HandController) -> None:
        """After GESTURE_CONFIRM_FRAMES with hand, transition IDLE→CURSOR."""
    ...
    def test_drag_on_sustained_pinch_with_movement(self, controller: HandController) -> None:
        ...
        # Pinch at a far position (large movement)
        pinch2 = _make_pinch_hand(200, 100)
    ...
    def test_drag_release(self, controller: HandController) -> None:
        ...
        # Get to CURSOR → CLICK_DOWN → DRAGGING
        for _ in range(3):
            controller.update([hand_open])
        pinch1 = _make_pinch_hand(320, 240)
        controller.update([_make_hand_result(pinch1)])
